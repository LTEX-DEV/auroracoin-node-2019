...
var BASE_PACKAGE = {
  description: 'A full Auroracoin node build with Bitcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'auroracoin': 'git://github.com/LTEX-DEV/bitcore-lib-2019.git',
    'auroracoin-node': 'git://github.com/LTEX-DEV/auroracoin-node-2019.git'
  }
};

/**
 * Will create a directory and auroracoin.conf file for Auroracoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createAuroracoinDirectory(datadir, done) {
 ...
 * @param {String} options.datadir - The auroracoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/auroracoin-node.json', configJSON);
      if (!isGlobal) {
 ...
 * auroracoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the auroracoin node configuration directory
 * @param {String} options.datadir - The path to the auroracoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the auroracoin-node directory and configuration
    ...
    function(next) {
      // Setup the auroracoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createAuroracoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },
