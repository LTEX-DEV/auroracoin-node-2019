...
var BASE_PACKAGE = {
  description: 'A full DigiByte node build with Bitcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'digibyte': '^' + bitcore.version,
    'digibyte-node': version
  }
};

/**
 * Will create a directory and digibyte.conf file for DigiByte.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createDigiByteDirectory(datadir, done) {
 ...
 * @param {String} options.datadir - The digibyte database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/digibyte-node.json', configJSON);
      if (!isGlobal) {
 ...
 * digibyte configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the digibyte node configuration directory
 * @param {String} options.datadir - The path to the digibyte datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the digibyte-node directory and configuration
    ...
    function(next) {
      // Setup the digibyte directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createDigiByteDirectory(absDataDir, next);
      } else {
        next();
      }
    },
