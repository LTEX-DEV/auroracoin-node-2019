...
/**
 * Will return the path and default digibyte-node configuration. It will search for the
 * configuration file in the "~/.digibyte" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.digibyte');
  var defaultConfigFile = path.resolve(defaultPath, './digibyte-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['digibyted', 'web'];
