...
/**
 * Will return the path and default auroracoin-node configuration. It will search for the
 * configuration file in the "~/.auroracoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.auroracoin');
  var defaultConfigFile = path.resolve(defaultPath, './auroracoin-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['auroracoind', 'web'];
