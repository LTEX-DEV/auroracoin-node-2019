...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for auroracoind is necessary for this upgrade with the "reindex=1" auroracoin.conf option. \n' +
                 'There are changes necessary in both auroracoin.conf and auroracoin-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node/blob/auroracoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed auroracoin-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './auroracoin-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in auroracoin-node services directory
