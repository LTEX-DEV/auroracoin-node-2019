...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for digibyted is necessary for this upgrade with the "reindex=1" digibyte.conf option. \n' +
                 'There are changes necessary in both digibyte.conf and digibyte-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node/blob/digibyted/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed digibyte-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './digibyte-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in digibyte-node services directory
