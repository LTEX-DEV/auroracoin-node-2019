...
/**
 * Provides a friendly event driven API to digibyted in Node.js. Manages starting and
 * stopping digibyted as a child process for application support, as well as connecting
 * to multiple digibyted processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function DigiByte(options) {
  if (!(this instanceof DigiByte)) {
    return new DigiByte(options);
  }
...
util.inherits(DigiByte, Service);

DigiByte.dependencies = [];

DigiByte.DEFAULT_MAX_TXIDS = 1000;
DigiByte.DEFAULT_MAX_HISTORY = 50;
DigiByte.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
DigiByte.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
DigiByte.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
DigiByte.DEFAULT_SPAWN_RESTART_TIME = 5000;
DigiByte.DEFAULT_SPAWN_STOP_TIME = 10000;
DigiByte.DEFAULT_TRY_ALL_INTERVAL = 1000;
DigiByte.DEFAULT_REINDEX_INTERVAL = 10000;
DigiByte.DEFAULT_START_RETRY_INTERVAL = 5000;
DigiByte.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
DigiByte.DEFAULT_TRANSACTION_CONCURRENCY = 5;
DigiByte.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
DigiByte.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || DigiByte.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || DigiByte.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || DigiByte.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || DigiByte.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || DigiByte.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || DigiByte.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || DigiByte.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || DigiByte.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || DigiByte.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || DigiByte.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

DigiByte.prototype._initCaches = function() {
...
DigiByte.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
DigiByte.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
DigiByte.prototype.getPublishEvents = function() {
  return [
    {
...
DigiByte.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'digibyted/' + name, 'total:', this.subscriptions[name].length);
};

DigiByte.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'digibyted/' + name, 'total:', this.subscriptions[name].length);
};

DigiByte.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'digibyted/addresstxid', 'total:', _.size(this.subscriptions.address));
};

DigiByte.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'digibyted/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
DigiByte.prototype.unsubscribeAddressAll = function(emitter) {
...
DigiByte.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = DigiByte.DEFAULT_CONFIG_SETTINGS;
...
DigiByte.prototype._parseDigiByteConf = function(configPath) {
  var options = {};
...
DigiByte.prototype._expandRelativeDatadir = function() {
...
DigiByte.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in digibyted config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in digibyted config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in digibyted config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './digibyte.conf');

  log.info('Using digibyte config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseDigiByteConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseDigiByteConf(networkConfigPath));
  }
...
DigiByte.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of digibyte-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of digibyte-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of digibyte-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to digibyted from digibyte. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from digibyted. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from digibyted. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in digibyte.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that digibyted is undergoing a reindex. ' +
             ...
             'of digibyte-node services will start.');
    node._reindex = true;
  }
};

DigiByte.prototype._resetCaches = function() {
...
DigiByte.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
DigiByte.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

DigiByte.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('DigiByte Daemon Ready');
          callback();
        });
      });
...
DigiByte.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 14022
  };
...
DigiByte.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/digibyte.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/digibyte.conf';
    }
  }
  return networkPath;
};

DigiByte.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
DigiByte.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('digibyted/hashblock', message.toString('hex'));
    }
  }

};

DigiByte.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
DigiByte.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('DigiByte Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
DigiByte.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
DigiByte.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('digibyted/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
DigiByte.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('digibyted/rawtransaction', message.toString('hex'));
    }
...
DigiByte.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || DigiByte.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

DigiByte.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
DigiByte.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
DigiByte.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('DigiByte Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || DigiByte.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

DigiByte.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
DigiByte.prototype._stopSpawnedDigiByte = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/digibyted.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned digibyte process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean digibyte process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
DigiByte.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedDigiByte(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting digibyte process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('DigiByte process unexpectedly exited with code:', code);
        log.warn('Restarting digibyte child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Digibyte process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn digibyted.'));
      }
...
DigiByte.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to digibyted.'));
    }
 ...
 */
DigiByte.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('DigiByte configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
DigiByte.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
DigiByte.prototype.syncPercentage = function(callback) {
  var self = this;
...
DigiByte.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
DigiByte.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
DigiByte.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
DigiByte.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
DigiByte.prototype._getConfirmationsDetail = function(transaction) {
...
DigiByte.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
DigiByte.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
DigiByte.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A digibyte transaction id
 * @param {Function} callback
 */
DigiByte.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
DigiByte.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
DigiByte.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
DigiByte.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      if (!tx.coinbase) {
        tx.inputSatoshis += bitcore.Unit.fromBTC(input.value).toSatoshis();
      }
      var script = null;
      var scriptAsm = null;
      if (input.scriptSig) {
      ...
      tx.inputs.push({
        ...
        satoshis: _.isUndefined(bitcore.Unit.fromBTC(input.value).toSatoshis()) ? null : bitcore.Unit.fromBTC(input.value).toSatoshis()
      });
    }
  }
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      tx.outputSatoshis += bitcore.Unit.fromBTC(out.value).toSatoshis();
      var address = null;
      if (out.scriptPubKey && out.scriptPubKey.addresses && out.scriptPubKey.addresses.length === 1) {
        address = out.scriptPubKey.addresses[0];
      }
      tx.outputs.push({
        satoshis: bitcore.Unit.fromBTC(out.value).toSatoshis(),
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.getInfo = function(callback) {
  var self = this;
...
DigiByte.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
DigiByte.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('digibyted spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('digibyted process did not exit'));
      }
