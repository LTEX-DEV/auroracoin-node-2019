...
Auroracoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      if (!tx.coinbase) {
        tx.inputSatoshis += bitcore.Unit.fromDGB(input.value).toSatoshis();
      }
      var script = null;
      var scriptAsm = null;
      if (input.scriptSig) {
      ...
      tx.inputs.push({
        ...
        satoshis: _.isUndefined(bitcore.Unit.fromDGB(input.value).toSatoshis()) ? null : bitcore.Unit.fromDGB(input.value).toSatoshis()
      });
    }
  }
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      tx.outputSatoshis += bitcore.Unit.fromDGB(out.value).toSatoshis();
      var address = null;
      if (out.scriptPubKey && out.scriptPubKey.addresses && out.scriptPubKey.addresses.length === 1) {
        address = out.scriptPubKey.addresses[0];
      }
      tx.outputs.push({
        satoshis: bitcore.Unit.fromDGB(out.value).toSatoshis(),
