...
/**
 * Provides a friendly event driven API to auroracoind in Node.js. Manages starting and
 * stopping auroracoind as a child process for application support, as well as connecting
 * to multiple auroracoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Auroracoin(options) {
  if (!(this instanceof Auroracoin)) {
    return new Auroracoin(options);
  }
...
util.inherits(Auroracoin, Service);

Auroracoin.dependencies = [];

Auroracoin.DEFAULT_MAX_TXIDS = 1000;
Auroracoin.DEFAULT_MAX_HISTORY = 50;
Auroracoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Auroracoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Auroracoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Auroracoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Auroracoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Auroracoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Auroracoin.DEFAULT_REINDEX_INTERVAL = 10000;
Auroracoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Auroracoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Auroracoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Auroracoin.DEFAULT_CONFIG_SETTINGS = {
  rpcport:12341,
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  ...
  timestampindex: 1,
  spentindex: 1,
  zmqpubrawtx: 'tcp://127.0.0.1:21402',
  zmqpubhashblock: 'tcp://127.0.0.1:21402',
...
Auroracoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Auroracoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Auroracoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Auroracoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Auroracoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Auroracoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Auroracoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Auroracoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Auroracoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Auroracoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Auroracoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Auroracoin.prototype._initCaches = function() {
...
Auroracoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Auroracoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Auroracoin.prototype.getPublishEvents = function() {
  return [
    {
...
Auroracoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'auroracoind/' + name, 'total:', this.subscriptions[name].length);
};

Auroracoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'auroracoind/' + name, 'total:', this.subscriptions[name].length);
};

Auroracoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'auroracoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Auroracoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'auroracoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Auroracoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Auroracoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Auroracoin.DEFAULT_CONFIG_SETTINGS;
...
Auroracoin.prototype._parseAuroracoinConf = function(configPath) {
  var options = {};
...
Auroracoin.prototype._expandRelativeDatadir = function() {
...
Auroracoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in auroracoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in auroracoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in auroracoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './auroracoin.conf');

  log.info('Using auroracoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseAuroracoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseAuroracoinConf(networkConfigPath));
  }
...
Auroracoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of auroracoin-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of auroracoin-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of auroracoin-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to auroracoind from auroracoin. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from auroracoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from auroracoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in auroracoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that auroracoind is undergoing a reindex. ' +
             ...
             'of auroracoin-node services will start.');
    node._reindex = true;
  }
};

Auroracoin.prototype._resetCaches = function() {
...
Auroracoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Auroracoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Auroracoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Auroracoin Daemon Ready');
          callback();
        });
      });
...
Auroracoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 12341
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 12342;
  }
  return networkOptions;
};

Auroracoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/auroracoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/auroracoin.conf';
    }
  }
  return networkPath;
};

Auroracoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Auroracoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('auroracoind/hashblock', message.toString('hex'));
    }
  }

};

Auroracoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Auroracoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Auroracoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Auroracoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Auroracoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('auroracoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Auroracoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('auroracoind/rawtransaction', message.toString('hex'));
    }
...
Auroracoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Auroracoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Auroracoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Auroracoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Auroracoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Auroracoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Auroracoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Auroracoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Auroracoin.prototype._stopSpawnedAuroracoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/auroracoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned auroracoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean auroracoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Auroracoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedAuroracoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting auroracoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Auroracoin process unexpectedly exited with code:', code);
        log.warn('Restarting auroracoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Auroracoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new AuroracoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 12341,
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn auroracoind.'));
      }
...
Auroracoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to auroracoind.'));
    }
 ...
 */
Auroracoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Auroracoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Auroracoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Auroracoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Auroracoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Auroracoin.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Auroracoin.prototype._getConfirmationsDetail = function(transaction) {
...
Auroracoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Auroracoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Auroracoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A auroracoin transaction id
 * @param {Function} callback
 */
Auroracoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Auroracoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Auroracoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Auroracoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      if (!tx.coinbase) {
        tx.inputSatoshis += bitcore.Unit.fromAUR(input.value).toSatoshis();
      }
      var script = null;
      var scriptAsm = null;
      if (input.scriptSig) {
      ...
      tx.inputs.push({
        ...
        satoshis: _.isUndefined(bitcore.Unit.fromAUR(input.value).toSatoshis()) ? null : bitcore.Unit.fromAUR(input.value).toSatoshis()
      });
    }
  }
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      tx.outputSatoshis += bitcore.Unit.fromAUR(out.value).toSatoshis();
      var address = null;
      if (out.scriptPubKey && out.scriptPubKey.addresses && out.scriptPubKey.addresses.length === 1) {
        address = out.scriptPubKey.addresses[0];
      }
      tx.outputs.push({
        satoshis: bitcore.Unit.fromAUR(out.value).toSatoshis(),
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.getInfo = function(callback) {
  var self = this;
...
Auroracoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Auroracoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('auroracoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('auroracoind process did not exit'));
      }
